...
import {ProblemBrief, MethodWorkflow} from './Overview.jsx';
...
export default function App() {
  ...
  return <div className="app-shell">
    <nav className="side-rail" aria-label="기본 탐색"><a className="brand-symbol" href="/" aria-label="내일의집 홈"><Warehouse size={24} /></a><span className="rail-line" /><button className={!showSaved ? 'rail-button active' : 'rail-button'} aria-label="네트워크 화면" title="네트워크" onClick={() => setShowSaved(false)}><Network size={21} /></button><button className={showSaved ? 'rail-button active' : 'rail-button'} disabled={!scenario || busy} aria-label="시나리오 보관함" title="시나리오 보관함" onClick={() => setShowSaved(true)}><FolderOpen size={21} /></button><div className="rail-bottom"><span>INU</span><small>v1.0</small></div></nav>
    <div className="main-shell"><header className="topbar"><div className="brand"><b>내일의집<span>물류시설설계 및 시뮬레이션</span></b><i /><span>인천대학교 동북아물류대학원 · 박사과정 202653005 이현호</span></div><div className="topbar-right"><span className="local-badge"><i />LOCAL WORKSPACE</span><button className="icon-button" aria-label="사용 안내" title="사용 안내" onClick={() => tell('① 거점을 지도에서 편집 ② 정책·차량·수요 설정 ③ 최적화 실행 ④ 결과 확인 후 시나리오 저장·비교. 모든 입력 변경은 기존 결과를 초기화합니다.')}><BookOpen size={19} /></button></div></header>
      <main className="workspace">
        <div className="page-heading"><ProblemBrief /><span className="workspace-badge"><span className="dot green" />66개 수요지역 · 3품목 · 3차종</span></div>
        <MethodWorkflow />
        ...
        {loading || !scenario ? <div className="loading-screen"><Network size={40} /><h2>{loading ? '네트워크 데이터를 준비하고 있습니다' : '데이터를 불러오지 못했습니다'}</h2><p>원본 수요와 후보 DC를 확인합니다.</p>{!loading && <button className="button" onClick={loadDefault}>다시 불러오기</button>}</div> : <>
          ...
          <footer className="workspace-footer"><span>물류시설설계 및 시뮬레이션 <b>·</b> 내일의집 네트워크 설계 과제</span><span>Haversine 직선거리 · 일평균 정상상태 수요 · 패널티 허용 시 경제적 미충족 가능</span><button className="text-button" disabled={busy} onClick={loadDefault}><RotateCcw size={12} />원본 시나리오 복원</button></footer>
