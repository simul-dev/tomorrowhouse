  ...
  return <section className={`map-panel ${addMode ? 'map-adding' : ''}`} aria-label="물류 네트워크 지도">
    ...
    <div className="map-body">
      <MapContainer center={[37.53, 127.04]} zoom={9} zoomControl={true} scrollWheelZoom={true} className="network-map">
        ...
        {current && hasCoords(current) && radius > 0 && <Circle center={[current.lat, current.lon]} radius={radius * 1000} pathOptions={{color: '#d49a1c', weight: 1, dashArray: '5 6', fillOpacity: .025, interactive: false}} />}
        {layers.connections && period?.assignments.map(a => {
          ...
          return <Polyline key={a.customer_id} positions={[[f.lat, f.lon], [c.lat, c.lon]]} pathOptions={{color: a.quantities.premium ? '#c8860d' : '#2f6fd0', opacity: .5, weight: Math.min(6, 1 + cbm(a.quantities) / 65)}}><Tooltip>{f.name} → {c.name}<br />{fmt(total(a.quantities))}개 · 편도 {fmt(a.distance_km, 1)}km</Tooltip></Polyline>;
        })}
        {scenario.customers.filter(hasCoords).map(c => <React.Fragment key={c.id}>
          {layers.premium && c.demand.premium > 0 && <CircleMarker center={[c.lat, c.lon]} radius={Math.max(6, Math.min(13, 5 + Math.sqrt(c.demand.premium) / 3))} pathOptions={{color: '#e0a020', weight: 1.2, fillColor: '#e0a020', fillOpacity: .12, interactive: false}} />}
          {layers.demand && <CircleMarker center={[c.lat, c.lon]} radius={Math.max(3, Math.min(7, 2 + Math.sqrt(total(c.demand)) / 9))} pathOptions={{color: '#fff', weight: 1, fillColor: '#3273cf', fillOpacity: .9}}><Tooltip>{c.name} · 기준 {fmt(total(c.demand))}개</Tooltip><Popup><div className="map-popup"><small>고정 수요지역</small><h3>{c.name}</h3><p>인구 {fmt(c.population)}명</p>{[['large', '대형'], ['small', '소형'], ['premium', '프리미엄']].map(([key, name]) => <div className="popup-row" key={key}><span>{name}</span><strong>{fmt(Math.floor(c.demand[key] * multiplier + .5))}개</strong></div>)}<small>적용 배수 {fmt(multiplier, 3)} · {fmt(c.lat, 4)}, {fmt(c.lon, 4)}</small></div></Popup></CircleMarker>}
        </React.Fragment>)}
